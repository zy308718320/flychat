<template>
  <div class="wrap" :style="{ visibility: isNoChat ? 'hidden' : 'visible' }">
    <div class="expression-wrap" :class="{ 'expression-wrap-hidden': !isShowExpression}" @click.stop=";">
      <div class="expression-header">
        <div v-for="(expression, index) in expressions" :key="'expression' + index" class="expression-item" :class="{ 'expression-item-selected': expressionIndex === index}" @click.stop="expressionIndex = index">
          {{ expression.title }}
        </div>
      </div>
      <div class="expression-content list-wrap">
        <div v-for="(row, rIndex) in expressions[expressionIndex].items" :key="'eRow' + rIndex" class="expression-row">
          <a v-for="(col, cIndex) in row" :key="'eCol' + cIndex" :title="col.title" class="expression-icon" :style="{'background': `url(${expressions[expressionIndex].img}) ${col.y}px ${col.x}px`}" @click="handleAddExpression(rIndex, cIndex)"></a>
        </div>
      </div>
    </div>
    <div class="menu">
    ...
    <pre id="content-input" class="content-input list-wrap" contenteditable="true" @paste="handleCopy" @keypress.enter.exact.prevent.stop="handleSend" @keydown.ctrl.enter.prevent.stop="handleLineFeed" @click="handleEditorFocus" @keyup="handleEditorFocus"></pre>
    <div class="action">
  ...
  for (let i = 0; i < ctnInput.childNodes.length; i++) {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (node.nodeName.toUpperCase() === 'IMG') {
        ...
        let size
        if (dataset.type === 'qqfaces') {
          size = 20
        } else {
          size = 28
        }
        ctn.push(`<img src="${spacer}" class="expression-icon-small" style="width: ${size}px; height: ${size}px; display: inline-block; vertical-align: middle; background: url(${dataset.url}) ${dataset.y}px ${dataset.x}px" />`)
  ...
  methods: {
    ...
    handleAddExpression (rIndex, cIndex) {
      ...
      document.execCommand(
        'insertHTML',
        false,
        `<img src="${spacer}" data-url="${this.expressions[this.expressionIndex].smallImg}" data-type="${type}" data-x="${sx}" data-y="${sy}" class="expression-icon-small" style="width: ${size}px; height: ${size}px; display: inline-block; vertical-align: middle; background: url(${this.expressions[this.expressionIndex].smallImg}) ${sy}px ${sx}px" />`
      )
    ...
    async handleSend () {
      ...
      const { currentChatId } = this.$store.state
